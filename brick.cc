...
int drawBrickShadow(BRICK * b,int x,int y,SDL_Surface* shadow,SDL_Surface* lpddsback)
{
	if(b->state==normal)
	{
...
int drawBrick(BRICK * b,int x,int y,SDL_Surface* lpddsback)
{
	if(b->state==normal)
	{
		DDraw_Draw_Surface(brickTextures[b->texture][b->type],
						   x,y,16,16,lpddsback,1);
	}
	else if(b->state==weakening)
	{

		DDraw_Draw_Surface(brickTextures[WEAKENING_TEXTURE][b->type],
							x,y,16,16,lpddsback,1);
		b->timer-=2;
		if(b->timer<=0)
		{
	...
	else if(b->state==breaking)
	{
		int xsize,ysize;
		int xoff,yoff;
		if(b->type==BRICK_HOR||b->type==BRICK_LEND||b->type==BRICK_REND)
		{
			xsize=16;
			ysize=b->timer;
			xoff=0;
		...
		DDraw_DrawSized_Surface(brickTextures[WEAKENING_TEXTURE][b->type],
							x+xoff,y+yoff,16,16,xsize,ysize,lpddsback,1);
		b->timer--;
		if(b->timer<=0)
			b->state=invalid;
	}
...
int hitTheBricks(BRICK bricks[BHEIGHT][BWIDTH],BALL * ball)
{
	...
	/*
		New diagonal checks
	*/
	else if(hitBrick(bricks,locx+3,
							locy+3))
	{
	...
	else if(hitBrick(bricks,locx+12,
							locy+3))
	{
		//ball->speed=0;
		ball->dir.y*=-1;
                       ...
                       int r,int g,int b)          // transparency flag
{
  ...
  //ddbltfx.dwFillColor = _RGB16BIT565((r>=31)?r:r+1,(g>=31)?g:g+1,(b>=31)?b:b+1); 
  int dwFillColor = SDL_MapRGB(lpdds->format, (int)((((float)r)*0.7f)+((float)(0.3f*0xf1))),
...
//ddbltfx.dwFillColor = _RGB16BIT565((r>=31)?r:r+1,(g>=31)?g:g+1,(b>=31)?b:b+1); 
	dwFillColor = SDL_MapRGB(lpdds->format,  (int)((((float)r)*0.85f)+((float)(0.15f*0xf1))),
	...
	*/
	...
	//ddbltfx.dwFillColor = _RGB16BIT565((r>=1)?r-1:r,g,(b>=1)?b-1:b); 
        dwFillColor = SDL_MapRGB(lpdds->format, (int)(((float)r)*0.8f),(int)(((float)g)*0.8f),(int)(((float)b)*0.8f));
	...
	dwFillColor = SDL_MapRGB(lpdds->format, (int)(((float)r)*0.6f),(int)(((float)g)*0.6f),(int)(((float)b)*0.6f));
