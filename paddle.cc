...
int checkForCollision(PADDLE* p, BALL* b) {
  ...
  if ((b->loc.x + BALL_RAD > p->loc.x) &&
      (b->loc.x + BALL_RAD < (p->loc.x + (p->width << 4)))) {
    if (((b->loc.y + 16) >= p->loc.y) && ((b->loc.y + 8) <= p->loc.y)) {
      float r = rand() % 10 + 1;
      b->dir.y *= -1;
      r = -1 * r * p->dir.x / 10.0f;
      b->dir.x += r;
  ...
  } else if ((b->loc.x + 16 >= p->loc.x) && (b->loc.x < p->loc.x) &&
             b->dir.x > 0) {
    if (((b->loc.y + 8) >= p->loc.y) && (p->loc.y <= p->loc.y + 8)) {
      b->dir.x *= -1;
    }
  } else if ((b->loc.x <= (p->loc.x + (p->width << 4))) &&
             (b->loc.x > p->loc.x) && b->dir.x < 0) {
    if (((b->loc.y + 8) >= p->loc.y) && (p->loc.y <= p->loc.y + 8)) {
      b->dir.x *= -1;
    }
  }
