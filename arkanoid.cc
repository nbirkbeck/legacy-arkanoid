...
SDL_Surface* createShadow(SDL_Surface* source, int width, int height) {
  SDL_Surface* src32 =
      SDL_CreateRGBSurface(0, source->w, source->h, 32, 0, 0, 0, 0);
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++) {
      if (source_row[x] != black) {
        ...
        dest_ptr[y * dest_stride + x] = black;
    }
  }
...
int drawBallShadow(BALL* b) {
  ...
  DDraw_Draw_Surface(ballShadowTexture, (int)b->loc.x + 8, (int)b->loc.y + 8,
                     16, 16, surface, 1);
  return 1;
}
...
int drawBall(BALL* b) {
  ...
  if (b->powerBall) {
    b->frame++;
    DDraw_Draw_Surface(pballTextures[((b->frame >> 2) % 4)], (int)b->loc.x,
                       (int)b->loc.y, 16, 16, surface, 1);
  } else
    DDraw_Draw_Surface(ballTexture, (int)b->loc.x, (int)b->loc.y, 16, 16,
                       surface, 1);
...
int drawPupShadow(POWERUP* p) {
  if (p->active) {
    DDraw_Draw_Surface(pupShadowTextures[p->type], (int)p->loc.x + 8,
                       (int)p->loc.y + 8, 32, 16, surface, 1);
  }
  return 1;
}
...
int drawPup(POWERUP* p) {
  if (p->active) {
    DDraw_Draw_Surface(pupTextures[p->type], (int)p->loc.x, (int)p->loc.y, 32,
                       16, surface, 1);
  }
  return 1;
}
...
int drawLasers(LASER* l) {
  if (l == NULL)
    return 0;
  drawLasers(l->next);
  DDraw_Draw_Surface(paddle.bullet, (int)l->loc.x, (int)l->loc.y, 8, 8, surface,
                     1);
  return 1;
}
...
int drawPaddle(PADDLE* p) {
  ...
  if (p->lasers) {
    DDraw_Draw_Surface(p->textures[LEFT_LASER], (int)p->loc.x, (int)p->loc.y,
                       16, 32, surface, 1);
  } else
    DDraw_Draw_Surface(p->textures[LEFT], (int)p->loc.x, (int)p->loc.y, 16, 32,
                       surface, 1);
  while (i < (p->width - 1)) {
    DDraw_Draw_Surface(p->textures[MIDDLE], (int)p->loc.x + 16 * i,
                       (int)p->loc.y, 16, 32, surface, 1);
    i++;
  }
  if (p->lasers)
    DDraw_Draw_Surface(p->textures[RIGHT_LASER], (int)p->loc.x + 16 * i,
                       (int)p->loc.y, 16, 32, surface, 1);
  else
    DDraw_Draw_Surface(p->textures[RIGHT], (int)p->loc.x + 16 * i,
                       (int)p->loc.y, 16, 32, surface, 1);
  return 1;
}
...
int Game_Main() {
  ...
  if (!paused) {
    ...
    drawFont(surface, &font, "Paused", 320 - (getLength(&font, "Paused") >> 1),
             240);
...
int loadSounds() {
  breakSounds[0] =
      FSOUND_Sample_Load(FSOUND_FREE, "./sounds/break0.wav", FSOUND_NORMAL, 0);
  breakSounds[1] =
      FSOUND_Sample_Load(FSOUND_FREE, "./sounds/break1.wav", FSOUND_NORMAL, 0);
  return 1;
}
int freeSounds() {
...
int playBreakSound() {
  channel += 1;
  channel %= 8;
  FSOUND_PlaySound(channel, breakSounds[1]);
  return 1;
}
#endif
...
int ballFunc(char* str) {
  ...
  if (sscanf(ptr, "%f %f %f", &ballHead->dir.x, &ballHead->dir.y,
             &ballHead->speed) == 3) {
  }
...
int Game_Init() {
  ...
  SDL_FillRect(background, nullptr,
               SDL_MapRGB(background->format, 0, 102, 102));
  ...
  char filenames[NUM_PUPS][32] = {
      "sticky",   "width",      "lasers", "ball", "powerball",
      "megaball", "megalasers", "warp",   "life", "shrink",
      "wonky",    "reverse",    "death"};
  i = 0;
  while (i < NUM_PUPS) {
    char actualName[255];
